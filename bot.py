...
class App(customtkinter.CTk):
    ...
    def abrir_janela_cadastro(self):
        ...
        janela_cadastro.geometry(f"{500}x{300}")
        janela_cadastro.resizable(True, True)

        customtkinter.CTkLabel(janela_cadastro, text="Nome de Usuário:").grid(row=2, column=0)
        self.novo_username_entry = customtkinter.CTkEntry(janela_cadastro)
        self.novo_username_entry.grid(row=2, column=1)

        customtkinter.CTkLabel(janela_cadastro, text="Senha:").grid(row=3, column=0)
        self.nova_password_entry = customtkinter.CTkEntry(janela_cadastro, show="*")
        self.nova_password_entry.grid(row=3, column=1)

        botao_salvar_cadastro = customtkinter.CTkButton(janela_cadastro, text="Salvar", command=self.salvar_usuario)
        botao_salvar_cadastro.grid(row=4, column=0, pady=10)
...
class ChatbotWindow(customtkinter.CTk):
    ...
    def salvar_usuario(self):
        ...
        nome = self.novo_username_entry.get()
        senha = self.nova_password_entry.get()
        self.conversa_cadastro.insert(customtkinter.END,f'Cadastrado com sucesso: Nome {nome} e senha {senha}')
